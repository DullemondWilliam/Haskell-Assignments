...
evalExp env (Cond f) = evalTuple env f
...
--evalExp env (SLet ids exps e)= error "I dont know"

evalExp env (SLet [] [] exp) = evalExp env exp
evalExp env (SLet (i:ids)(e:exps) exp) = evalExp  (extendEnv[i] [(evalExp env e)] env ) (SLet ids exps exp)

--evalExp  EmptyEnv                      (SLet ids exps e') = evalExp (ExtendedEnv    (Frame (ids) ((wolfCow' (ExtendedEnv    (Frame [] []) emptyEnv) ids exps [] []))) emptyEnv)(Let ids exps e')
--evalExp (ExtendedEnv    (Frame a b) e) (SLet ids exps e') = evalExp (ExtendedEnv    (Frame (ids) ((wolfCow' (ExtendedEnv    (Frame a b) e) ids exps [] []))) e)(Let ids exps e')
--evalExp (RecExtendedEnv (Frame a b) e) (SLet ids exps e') = evalExp (RecExtendedEnv (Frame (ids) ((wolfCow' (RecExtendedEnv (Frame a b) e) ids exps [] []))) e)(Let ids exps e')
...
hasBinding EmptyEnv i = False;
hasBinding (ExtendedEnv (Frame a wb) e) i   = (hasBindFrame a i)||(hasBinding e i)
hasBinding (RecExtendedEnv (Frame a b) e) i = (hasBindFrame a i)||(hasBinding e i)


--  if (all(show(i)!=show()) a)
--    then False 
--    else True 


hasBindFrame :: [Id] -> Id -> Bool
hasBindFrame [] i = False
hasBindFrame (x:xs) i = if(show(x)==show(i))
                        then True
                        else hasBindFrame xs i
...
evalTuple:: Env -> [(Exp,Exp)]-> Val
evalTuple env [] = (Int 0)
evalTuple env ((a,b):xs) = if(evalVal (evalExp env a))
                       then (evalExp env b)
                       else (evalTuple env xs)

evalVal:: Val -> Bool
evalVal (Int i) = if(i == 0)
                  then False
                  else True
evalVal (Bool b) = b
evalVal (ProcVal a b c) = error "i dont understand"
...
boundIds :: Env -> [Id]
boundIds (EmptyEnv ) = []
boundIds (ExtendedEnv (Frame i v) e)    = i++ boundIds(e)
boundIds (RecExtendedEnv (Frame i v) e) = i++ boundIds(e)



--evalExp env (SLet idl expl e) = undefined
...
--Program (Let ["f"] [                                                 ] (                                                                                                                                                                                                                                                                                                          )
--                     Proc ["x"] (                                   )   Let ["g"] [Proc ["h","y"] (                                                           )] (                                                                                                                                                                                                               )
 --                                PrimitiveApp Add [Var "x",Number 3]                               PrimitiveApp Succ [                                       ]    Let ["fact"] [                   ] [                                                                                                                      ] (Var "f")] (App (Var "g") [Var "fact",Number 17]))
  --                                                                                                                    App (Var "h") [                       ]                   Letrec ["f"] [["n"]    If (                             )(Number 1)(                                                                       )
  --                                                                                                                                  App (Var "f") [Var "y"]                                               PrimitiveApp IsZero [Var "n"]            PrimitiveApp Mult [                                                   ]
  --                                                                                                                                                                                                                                                                    Var "n",App (Var "f") [                           ] 
  --                                                                                                                                                                                                                                                                                            PrimitiveApp Pred [Var "n"]                                  
-- Question 4
--
-- The Exp datatype (and parser) includes an "slet" constructor that
-- is syntactically identical to "let" except for using the keyword
...
{-}
sharkBear::Env -> Id-> Val-> Env
sharkBear (EmptyEnv ) i v= EmptyEnv
sharkBear (ExtendedEnv  (Frame ids vs) t) i v= ExtendedEnv (Frame (ids++[i])(vs++[v])) (sharkBear t i v)
sharkBear (RecExtendedEnv  (Frame ids vs) t) i v= RecExtendedEnv (Frame (ids++[i])(vs++[v])) (sharkBear t i v)
-}
--catDog:: Frame -> Env
--catDog (Frame i:ids e:exps)= 

--wolfCow:: Frame -> Id ->Exp
--wolfCow (Frame i:ids e:exps) a b = 

--wolfCow :: Env -> [Id]->[Exp] -> [Val]

--wolfCow (ExtendedEnv (Frame a b) e)    (l:ids) (x:exps)  = [(evalExp (ExtendedEnv (Frame a b) e) x )]   ++ (wolfCow (ExtendedEnv   (Frame (a++[l]) (b++[(evalExp (ExtendedEnv    (Frame a b) e) x )]) )e )ids exps)
--wolfCow (RecExtendedEnv (Frame a b) e) (l:ids) (x:exps)  = [(evalExp (RecExtendedEnv (Frame a b) e) x )]++ (wolfCow (RecExtendedEnv(Frame (a++[l]) (b++[(evalExp (RecExtendedEnv (Frame a b) e) x )])) e)ids exps)

--wolfCow  EmptyEnv _ _ = []
--wolfCow  env [] [] =  []
--wolfCow  env _ _ = error "not a matching number of ids and e"


--wolfCow' :: Env -> [Id]->[Exp] -> [Id]->[Val]->[Val]

--wolfCow' (ExtendedEnv (Frame a b) e)    (l:ids) (x:exps) tId tEx = [(evalExp (ExtendedEnv (Frame a b) e)    (Let tId tEx x) )]   ++ (wolfCow' (ExtendedEnv    (Frame (a++[l]) (b++[(evalExp (ExtendedEnv    (Frame a b) e) x )]) )e )ids exps (tId++[l]) (tEx++[(Let tId tEx x)]))
--wolfCow' (RecExtendedEnv (Frame a b) e) (l:ids) (x:exps) tId tEx = [(evalExp (RecExtendedEnv (Frame a b) e) (Let tId tEx x) )]   ++ (wolfCow' (RecExtendedEnv (Frame (a++[l]) (b++[(evalExp (RecExtendedEnv (Frame a b) e) x )]) )e )ids exps (tId++[l]) (tEx++[(Let tId tEx x)]))

--wolfCow'  EmptyEnv _ _ _ _= []
--wolfCow'  env [] [] _ _ =  []
--wolfCow'  env _ _ _ _= error "not a matching number of ids and e"



--evalExp env (SLet [] [] exp) = evalExp env exp
--evalExp env (SLet (id:ids)(expl:explist) exp) = evalExp(extendEnv[id] (evalExps env [expl]) env ) (SLet ids explist exp)
-- Question 5
--
...
q5Eg = show(run q3eg) ++ " != " ++ show(run' q3eg)
-- ++ "\n" ++ show(run eg6) ++ " != " ++ show(run' eg6)
...
slet0 = "slet x=3 y=+(x,1) in y"                               --4
slet1 = "let x=3 y=4 in slet z=2 f=proc() +(x,z) in (f)"       --5
slet2 = "let x=2 y=4 in slet x=succ(x) u=x z=y in +(x,+(u,z))" --10
slet3 = "slet x=8 in slet i =7 in slet a=x b=2 c=3 d=+(a,b) in d"
