...
fact n = do r <- newIORef n
            w <- newIORef n
            while
              (do {v <- readIORef r; writeIORef r (v-1) ; return (v >1)})

              (do {v <- readIORef r; t <- readIORef w; writeIORef w (t*(v)) })
            p <- readIORef w
            return(n,p)
...
fact' n =do r <- newRef n;t <- newRef n;
            while'
               (do {d <-readRef t;writeRef t (d-1); return (d > 1)})
               (do {v <- readRef r; d <-readRef t; writeRef r (v*d)})
            s <- readRef r;return(n,s)
