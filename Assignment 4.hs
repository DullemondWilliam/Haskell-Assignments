...
subst n m (Var x) = if (==) (show(x)) (show(n)) 
                    then m 
                    else Var x
subst n m (Nat x) = Nat x
subst n m (Op x) = Op x
subst n m (Lam x e) = if (==) (show(x)) (show(n)) 
                      then (Lam x e)
                      else Lam x (subst n m e)
subst n m (App e e') = App (subst n m e) (subst n m e')
...
isClosed :: Exp -> Bool
isClosed (Var x) = False
isClosed (Nat x) = True
isClosed (Op x) = True 
isClosed (Lam x e) = isClosed(subst x (Nat 0) e)
isClosed (App e e') = if(isClosed(e)==False||isClosed(e')==False)
                      then False
                      else True
...
eval (Nat x) = Nat x
eval (Op x ) = error "Operator by itSelf"
eval (Lam x e)= Lam x e--error "lambda by itself"

--handling app cases
--If There is a Lamba in the left
eval (App(Lam x e) (Var b))  = error "Lamba beside a variable"
eval (App(Lam x e) (Nat num))= eval(subst x (Nat num) e)

--App (App (Lam "x" (Lam "y" (exp))))) (Nat 0)) (Nat 2)

eval (App(App(Lam a (Lam b (c)))(Nat d))(Nat e)) = eval(subst a (Nat e) (subst b (Nat d) c))


eval (App(Lam x e) (Op b))   = error "Lamba beside a Operator"
eval (App(Lam x e) (Lam a b))= error "Lamba beside a Lambda"
eval (App(Lam x e) (App a b))= error "Lamba beside a App"

--If There is a Natual Number if the left
eval (App(Nat x) y) = error "Numbers arent suppose to be on the left"

--If there is an Op in the left
eval (App(Op _) (Var x))  = error "Operator on variable"


eval (App (App(App(Op IfZero)(a))(b))(c)) =if (eval (a)) == Nat 0
                                                      then eval b
                                                      else eval c
         
eval (App(Op Pred) (Nat y)) = Nat(y-1)
eval (App(Op Succ) (Nat y)) = Nat(y+1)

eval (App(Op _) (Nat y))  = error "UnDefinedOperator on a Nat" --eval x
eval (App(Op _) (Op b))   = error "Operator beside Operator" --eval x
eval (App(Op _) (Lam x e))= error "Operator on a Lam" --eval x


eval (App(Op z) (App x y))= eval(App(Op z)(eval(App x y))) 

--Catch case
eval (App e e') = eval(App (eval e) (eval e))
...
{-1*-}  , "succ (succ (pred 17))"
    -- 8
{-2-}  , "ifzero 0 (succ 0) (pred 3)"
    -- 1
{-3-}  , "ifzero 17 (succ 0) (pred 3)"
    -- 2
{-4*-}  , "(%x. x) 17"
    -- 17
{-5-}  , "(%x.%y. ifzero x y 17) 0 2"
    -- 2
{-6-}  , "(%x.%y. ifzero x y 17) 1 2"
    -- 17
{-7-}  , "(%x.%y. ifzero x y (%z. succ z)) 1 2 3"
    -- 4
{-8-}  , "(%x.%x.x) 1 2"
    -- 2
{-9-}  , "(%f.%x. f (f (f x))) (%f.%x. f (f (f x))) succ 0"
    -- 27
  ]
