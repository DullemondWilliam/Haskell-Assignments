...
evalExp env (Pair frt bck) =
  do{
      f <- (evalExp env frt);
      b <- (evalExp env bck);
      fR<- newRef f;
      bR<- newRef b;
      return (PairVal fR bR)
  }
  --error "asdasdSADS"
...
in this example.  Note: i can be assigned to in the body of the loop,
but any such value should be ignored at the start of each iteration
(see the example for1).A
-----------------------------------------------------------------------------}
evalCmd env (For id count top increment block)  =
 case count of
      Number c->do
                   p <- newRef (Int c);
                   boolStat <- evalExp env (PrimitiveApp  LessOrEq [count,top]);
                   newCount <- evalExp env (PrimitiveApp  Add  [count,increment]);
                   case newCount of 
                       Int nC -> do
                                   predStat <- evalExp env (PrimitiveApp  LessOrEq [(Number nC),top]);
                                   case boolStat of 
                                      Bool True -> case predStat of 
                                                     Bool True -> do{
                                                                   -- writeRef (applyEnv env id) (Int 12);
                                                                    x<-evalBlock (extendEnv [id] [p] env) block;
                                                                    evalCmd (extendEnv [id] [p] env) (For id (Number nC) top increment block)
                                                                    }
                                                     Bool False-> do{
                                                                     evalBlock (extendEnv [id] [p] env) block
                                                                   }    
                                      Bool False-> error"Hey what you think your doing running this terrible forLoop"



    -- case count of
     --     Number c ->
      --                 writeRef (applyEnv env id) (Int c);
            --evalCmd env (Flet id (Number d) block)
                                 --evalBlock env (wolfCow id (Number d) block)
  --evalCmd env (For id (Number d) top increment block
{- 
---------------------------------------------------------------------------
QUESTION 2.  "Fluid let."  This is a command of the form
...
x's reference is restored to the value it had at the start.
-----------------------------------------------------------------------------}
evalCmd env (Flet id exp b) = do{ y<-readRef (applyEnv env id);
                                  p<- evalExp env exp;
                                  writeRef (applyEnv env id) p;--(Int89567);
                                  m<-(evalBlock env b);
                                  writeRef (applyEnv env id) y ;
                                  return m
    
                                }
 -- mackIsHitler id exp e = let x = applyEnv e id 
 --                       in  do y <- readRef x;
 --                              z <- (evalExp e exp);
 --                              writeRef (applyEnv e id) (evalExp e exp)
 
 --  evalBlock env (wolfCow id e b)
----------------------------------------------------------------------------------
{-
QUESTION 3.  Mutable pairs (as in Lisp/Scheme).  These are pairs that
can have their components destructively modified.  Pairs are created
...
evalCmd env (Setfst epair exp) = 
  case epair of
        Pair frst scnd ->evalExp env (Pair exp scnd)
        ...
        App e es -> do{
                          ...
                          case y of 
                                 ...
                                 _ -> error"hahaha"--(show(y))
                        }

        PrimitiveApp snd e -> do{
                                   y <- evalExps env e;
                                   g <- applyPrimitive snd y;
                                  case g of 
                                         PairVal f s  ->do{
                                                             u <- evalExp env exp;
                                                             writeRef s u;
         ...
         {-} PrimitiveApp fst e -> do{
                                   y <- evalExps env e;
                                   g <- applyPrimitive fst y;
                                  case g of 
                                         PairVal f s  ->do{
                                                             u <- evalExp env exp;
                                                             writeRef f u;
...
-------------------------------------------------------------
evalCmd env (Setsnd epair exp) = 
  case epair of
        Pair frst scnd ->evalExp env (Pair frst exp)
...
applyPrimitive IsNull [v] = case v of 
...
{-}
catDog:: Id->Exp->Env-> Env
catDog id exp env = case env of 
                       EmptyEnv -> env
                       ExtendedEnv f e -> (ExtendedEnv (death env id exp f) e)
-}
--pigMan:: Env->Id->Exp->Frame->EVal
--pigMan env id exp (Frame [] [])= (Frame [] [])
--pigMan env id exp (Frame (i:ids)(e:exps)) =do {boolStat <- evalExp env exp; boolStat;}
                                             -- newCount <- evalExp env (PrimitiveApp  Add  [count,increment]);
                                             -- case newCount of
                                             -- Int d -> case boolStat of 
                                              --    Bool True -> evalCmd env (For id (Number d) top increment (wolfCow id (Number d) block))
                                               --   Bool False-> evalBlock env (wolfCow id (Number d) block)

                                                --(Frame [] [] )--if i /= id 
                                                  --then (Frame [] [] )--let x = (pigMan env id exp (Frame ids exps))
                                                       --  in case x of 
                                                        --    Frame a b ->(Frame ([i]++a)([e]++b))
                                               -- else (Frame [] [] )
                         ...
                         -- boolStat
                          ...
                          -}
                                      {-if i /= id
                                       then let x = (death env id exp (Frame ids exps))
                                              in case x of 
                                                Frame a b ->(Frame ([i]++a)([e]++b))
                                       else do h <- (evalExp env exp);h
                                           -} 
{-(Frame (i:ids)(e:exps))-}

{-}
 do boolStat <- evalExp env (PrimitiveApp  Less [count,top]);
     newCount <- evalExp env (PrimitiveApp  Add  [count,increment]);
       case newCount of
           Int d -> case boolStat of 
                    Bool True -> evalCmd env (For id (Number d) top increment (wolfCow id (Number d) block))
...
-}{-}
wolfCow :: Id->Exp->Block -> Block
{-wolfCow k z (Block (i:ids) (e:exps) p)= if i==k
                                    then (Block ([i]++ids) ([z]++exps) p)
                                    else let x = (wolfCow k z (Block ids exps p))
                                          in case x of 
                                             Block (ids) (exps) u ->(Block ([i]++ids) ([e]++exps) u)
-}
wolfCow k z b = b
-}


--mackIsHitler:: Id ->Exp-> Env-> Env
--mackIsHitler id exp e = let x = applyEnv e id 
--                        in  do y <- readRef x;
--                               z <- (evalExp e exp);
--                               writeRef x (evalExp e exp)
