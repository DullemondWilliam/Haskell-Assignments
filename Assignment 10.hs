...
dft p gs = case (search p 0 emptySubst gs) of
            Solution s -> [s]
            Branches b-> recBranch b



recBranch :: [Search] -> [Subst]
recBranch [] =[]
recBranch (s:se) = case s of
                    Solution a -> [a]++(recBranch se) 
                    Branches b -> (recBranch b)
...
bfs p gs = case (search p 0 emptySubst gs) of
            Solution s -> Just s
            Branches b-> help b

help :: [Search] -> Maybe Subst
help s =  case (checkLevel s)of
            Nothing -> help (goDeeper s)
            Just a -> Just a  

goDeeper :: [Search] -> [Search]
goDeeper [] = []
goDeeper (s:se) =case s of
                  Solution a -> error "THis is wrong"
                  Branches b -> b++(goDeeper se)
   

checkLevel :: [Search] -> Maybe Subst
checkLevel [] =Nothing
checkLevel (s:se) = case s of
                    Solution a -> Just a 
                    Branches b -> checkLevel se
